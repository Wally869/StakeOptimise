<script lang="ts">
	...
	import type {
		EvalDecentralisationPayload,
		EvalDecentralisationResponse,
		StakingResponse,
		Validator
	} from '$lib/types/pytypes';
	...
	onMount(async () => {
		...
		*/
		...
		try {
			...
			*/
			let stakes = $VALIDATORS_DATA.map((val: Validator) => {
			...
			DECENTRALIZATION_RATIO.set(rslt.decentralisation_ratio);
		} catch (e) {
			console.log(e);
		}
	});
...
<div class="bg-primary">
	<div class="m-auto max-w-4xl h-full py-16">
		<div class="grid grid-cols-1 gap-12 place-content-between">
			<!-- // TODO: add query network yield -->
			<NetworkInfo
				nb_validators={$VALIDATORS_DATA.length}
				network_yield={$NETWORK_YIELD}
				decentralisation_ratio={$DECENTRALIZATION_RATIO}
			/>
			<div class="w-full">
				<div
						...
						<div class="grid grid-cols-3">
							<div class="col-span-2">
								...
								<p class="text-gray-400">
									Maximize yield when staking and help decentralise governance
								</p>
							</div>
								...
								<div class="pb-4">
									{#if $STARGATE_SIGNING_CLIENT}
										<Button
											...
											variant="outline"
											on:click={() => {
												STARGATE_SIGNING_CLIENT.set(null);
											}}>Disconnect</Button
										>
									{:else}
										<Button
			...
			<div class="w-full">
				<div
					class="border border-2 p-8 w-full rounded-lg bg-primary-foreground"
				>
					<h1 class="font-semibold text-lg pb-4">About Stake Optimizer</h1>
					<div class="text-gray-800 grid grid-rows gap-4">
						<p>
							In Proof of Stake (PoS) cryptocurrency networks, it's crucial to evenly distribute
							stakes among validators for several key reasons:
						</p>
						<ul class="grid grid-rows gap-2">
							<li>
								- Security: A balanced stake distribution reduces the risk of centralization,
								enhancing network security.
							</li>
							<li>
								- Decentralization: Equal stakes support the network's goal of avoiding control by a
								single entity or small group.
							</li>
							<li>
								- Preventing Cartels: Balanced stakes discourage validator cartels that can undermine
								fair competition.
							</li>
							<li>
								- Incentives: Equal distribution encourages more validators to participate, increasing
								network resilience.
							</li>
							<li>
								- Resilience: A balanced distribution makes the network more resistant to targeted
								attacks.
							</li>
							<li>
								- Trustworthiness: Users trust networks that distribute stakes fairly, fostering user
								confidence.
							</li>
						</ul>
